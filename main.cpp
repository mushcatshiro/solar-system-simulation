...
constexpr float m2vp = (ViewportWidth) / (1000000000.f * 2); // pix/m
constexpr float m2vp2 = (ViewportWidth) / (440000000.f * 2);
constexpr float au2vp = (ViewportWidth) / (55.f * 2) ;  // pix/au
constexpr size_t ListLength = 64;
...
struct CelestialObj {
  ...
  CelestialObj(std::string n, float x_m, float y_m, float r_m, float m_kg, sf::Color c, bool st)
    : name(n), x(x_m), y(y_m), radius(r_m), mass(m_kg), color(c), shape(r_m * m2vp2), showTrail(st) {
    vx = 0.f;
    vy = 0.f;
    acceleration.x = 0.0f;
    acceleration.y = 0.0f;
...
CelestialObj earth{"earth", 0., 0., EarthRadius, EarthMass, sf::Color(159, 193, 100), true};
